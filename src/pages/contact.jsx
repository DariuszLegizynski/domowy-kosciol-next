				...
				<article className="grid grid-cols-1 justify-between">
					<section className="grid grid-cols-1 pt-8 px-8 justify-items-center lg:order-1 lg:grid-cols-2 lg:gap-x-24 lg:mx-auto lg:pb-8">
						<div className="fadeInFromLeft">
							...
							<strong className="p">{contactContent.name}</strong>
							<div className="flex py-2 pl-1.5 items-center">
								<IconItems type="phone" width="1.5rem" height="1.5rem" />
								<p className="pl-2">{contactContent.phoneNumber}</p>
							</div>
							<div className="flex items-center">
								<IconItems type="email" width="1.8rem" height="1.8rem" />
								<p className="pl-2">{contactContent.email}</p>
							</div>
						</div>
						<div className="pb-8 fadeInFromRight">
							<div className="h3 pt-8 pb-4 lg:pt-0">Gdzie można nas znaleźć?</div>
							<div>
					...
					<section className="grid grid-cols-1 pt-8 pb-24  justify-items-center bg-primary lg:order-2 lg:pt-16">
						...
						<form className="p-2">
							<div ref={addToRefs} className="mb-2 grid grid-rows-[auto_auto] max-w-full">
								<label className="p text-gold">Imię (wymagane)</label>
								<input className="p-1 w-full rounded" type="text" required />
							</div>
							<div ref={addToRefs} className="mb-2 max-w-full">
								<label className="p text-gold">Nazwisko</label>
								<input className="p-1 w-full rounded" type="text" />
							</div>
							<div ref={addToRefs} className="mb-2 max-w-full">
								<label className="p text-gold">E-mail (wymagane)</label>
								<input className="p-1 w-full rounded" type="email" required />
							</div>
							<div ref={addToRefs} className="mb-2 max-w-full">
								<label className="p text-gold">Telefon</label>
								<input className="p-1 w-full rounded" type="number" />
							</div>
							<div ref={addToRefs} className="mb-2 max-w-full">
								<label className="p text-gold">Wiadomość</label>
								<textarea className="p-1 w-full min-h-36 rounded" required />
							</div>
							<button ref={addToRefs} className="flex flex-col items-center mx-auto bg-gold text-primary px-12 py-3 mt-8 round" type="submit">
